...
> = TValue extends `${string}${ParamStart}${infer Param}${ParamEnd}${infer Rest}`
  ? Record<Param, ExtractPathParamType<Param, TParams>> & ExtractParamsFromString<Rest, TParams>
  : Record<never, never>

export type ParamsWithParamNameExtracted<TPath extends string> = {
  [K in keyof ExtractParamsFromString<TPath> as ExtractParamName<K>]?: Param
}
