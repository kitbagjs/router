...
describe("hooks", () => {
  ...
  test("functions are correctly typed", () => {
    ...
    expectTypeOf(router.onRejection).toEqualTypeOf<
      AddRejectionHook<BuiltInRejectionType, Routes>
    >();
  });
  ...
  test("to and from can be narrowed", () => {
    router.onBeforeRouteEnter((to, context) => {
      ...
      if (context.from?.name === "child") {
        ...
        expectTypeOf(context.from.params).toEqualTypeOf<{
          parentParam: string;
          childParam: string;
        }>();
      }
...
describe("route.matched.meta", () => {
  ...
  test("union type is preserved", () => {
    ...
    expectTypeOf(router.route.matched.meta).toEqualTypeOf<
      Readonly<{}> | Readonly<{ public: true }>
    >();
  });
