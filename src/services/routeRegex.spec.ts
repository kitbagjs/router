...
import {
  generateRouteHostRegexPattern,
  generateRoutePathRegexPattern,
  generateRouteQueryRegexPatterns,
  getParamName,
  regexCaptureAll,
  regexCatchAll,
  regexGreedyCatchAll,
  regexGreedyCaptureAll,
  replaceIndividualParamWithCaptureGroup,
  splitByMatches,
} from "@/services/routeRegex";
...
describe("generateRoutePathRegexPattern", () => {
  ...
  test("given path with params, returns value with params replaced with catchall", () => {
    ...
    const expected = new RegExp(
      `^parent/child/${regexCatchAll}/grand-child/${regexCatchAll}$`,
      "i",
    );
    expect(result.toString()).toBe(expected.toString());
  });
  ...
  test("given path with greedy param, uses greedy catch-all for that segment", () => {
    ...
    const expected = new RegExp(
      `^parent/${regexCatchAll}/${regexGreedyCatchAll}/${regexCatchAll}$`,
      "i",
    );
    expect(result.toString()).toBe(expected.toString());
  });
...
describe("generateRouteQueryRegexPatterns", () => {
  ...
  test("given query with required params, returns value with params replaced with catchall", () => {
    ...
    expect(result).toMatchObject([
      new RegExp(`dynamic=${regexCaptureAll}`),
      new RegExp("static=params"),
      new RegExp(`another=${regexCaptureAll}`),
    ]);
  });
...
describe("getParamName", () => {
  ...
  test.each([["foo"], ["?foo"], ["?:*foo"]])(
    "given string that is not param syntax, returns undefined",
    (paramName) => {
      const response = getParamName(paramName);
