...
export function getRoutesForRouter(
  ...
  const allRoutes = [...routes, ...plugins.map((plugin) => plugin.routes)];
...
function sortByDepthDescending(
  aRoute: Route & RouteInternal,
  bRoute: Route & RouteInternal,
): number {
