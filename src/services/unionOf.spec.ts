...
test("given no param returns value, throws InvalidRouteParamValueError", () => {
  ...
  const action: () => void = () =>
    getParamValue("foo", { param: union, isOptional: false, isGreedy: false });
...
test.each([23, "foo", true])("works with literal param of type %s", (value) => {
  const union = unionOf([value]);

  const result = getParamValue(value.toString(), {
    param: union,
    isOptional: false,
    isGreedy: false,
  });
