...
describe('hooks', () => {
  ...
  test('update is not given to leave hooks, which have no destination to update', () => {
    ...
    route.onBeforeRouteLeave((_to, context) => {
      expectTypeOf<keyof typeof context>().toEqualTypeOf<'from' | 'reject' | 'push' | 'replace' | 'abort' | 'context'>()
    })

    route.onAfterRouteLeave((_to, context) => {
      expectTypeOf<keyof typeof context>().toEqualTypeOf<'from' | 'reject' | 'push' | 'replace' | 'context'>()
    })
  })
