...
describe("getParamValue", () => {
  ...
  test("given Date constructor Param, returns for correct value for Date", () => {
    expect(getParamValue("2024-05-16T21:13:56.842Z", { param: Date })).toMatchObject(
      new Date("2024-05-16T21:13:56.842Z"),
    );
  ...
  test.each([[undefined], [""]])(
    "Given Optional Param and string without value, returns undefined",
    (stringWithoutValue) => {
  ...
  test.each([[undefined], [""]])(
    "Given Optional Param with default and string without value, returns default value",
    (stringWithoutValue) => {
      const paramTypes = [String, Number, Boolean, Date, JSON, /regexp/g, () => "getter"];

      for (const param of paramTypes) {
        expect(
          getParamValue(stringWithoutValue, { param: withDefault(param, "abc"), isOptional: true }),
        ).toBe("abc");
      }
    },
  );
...
describe("setParamValue", () => {
  ...
  test("given  Date constructor Param, returns for correct value for Date", () => {
    expect(setParamValue(new Date("2024-05-16T21:13:56.842Z"), { param: Date })).toBe(
      "2024-05-16T21:13:56.842Z",
    );
