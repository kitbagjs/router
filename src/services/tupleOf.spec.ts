...
test.each([{}, true, "", Infinity])(
  "given value is %s not an array, throws InvalidRouteParamValueError",
  (value) => {
...
test.each([["23"], ["true,23"]])(
  "given an array of params with invalid value %s, throws InvalidRouteParamValueError",
  (value) => {
    const array = tupleOf([Number, Boolean]);
