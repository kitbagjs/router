...
| <a id="render"></a> `render` | () => `Promise`\<[`ServerRenderResponse`](ServerRenderResponse.md)\> | Resolves once the router has nothing left to render: every prop and loader has settled, following any navigation one of them caused, so this waits for the next *full* render rather than for one navigation. Resolves with what a server should respond with. Awaiting `push` only waits for the route to commit; this also waits for its data. Requires the router to be created with the `ssr` option, and throws `SsrOptionRequiredError` without it. |
