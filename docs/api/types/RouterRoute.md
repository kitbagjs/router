...
| ------ | ------ | ------ | ------ |
| <a id="data"></a> `data` | `readonly` | `RouteDataOf`\<`TRoute`\[`"matches"`\]\> | What the route's loaders resolve to, keyed by loader name. A route whose only loader is unnamed exposes that loader's data here directly. |
| <a id="gettitle"></a> `getTitle` | `readonly` | `TRoute`\[`"getTitle"`\] | Title of the route. |
